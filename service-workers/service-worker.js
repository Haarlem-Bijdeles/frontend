...
const config = {
  ...
  staticCacheItems: [
    "/",
    "/wp-content/themes/haarlembijdeles/assets/css/style.css",
    "/wp-content/themes/haarlembijdeles/assets/scripts/main.js"
  ],
  offlineImage: `
...
self.addEventListener("install", event => {
  function onInstall(event) {
    ...
    return caches.open(cacheKey).then(cache => cache.addAll(staticCacheItems));
  }

  event.waitUntil(onInstall(event).then(() => self.skipWaiting()));
});
  ...
  function onActivate(event) {
    return caches.keys().then(cacheKeys => {
      const oldCacheKeys = cacheKeys.filter(
        key => key.indexOf(config.version) !== 0
      );
